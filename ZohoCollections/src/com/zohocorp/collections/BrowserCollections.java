...
class Browser {
	...
	{  
		...
		void visit(String visitUrl)
		{
			try
			{
				...
				if (Arrays.asList(extensionsArray).contains(extension))
				{
					...
					urlLinkedList.add(Reference.currentPosition,visitUrl);
					urlHashSet.add(visitUrl);
					urlLinkedHashSet.add(visitUrl);
					urlTreeSet.add(visitUrl);
				}
				else
				{
		...
		public void sortUrls()
		{
			...
			for(int i=0;i<urlLinkedHashSet.size();i++)
			{
				System.out.println(Collections.min(urlLinkedHashSetSort));
				urlLinkedHashSetSort.remove(Collections.min(urlLinkedHashSetSort));
			}
		...
		public void searchExtension(String extensionSeached) 
		{
			...
			String element;
			Iterator<String> iteratorUrl=urlHashSet.iterator(); 
			System.out.println("Seaching in Hash Set");
			while(iteratorUrl.hasNext())
			{
				element=iteratorUrl.next();
				extensionIndex=element.lastIndexOf(".");
				extension=element.substring(extensionIndex, element.length());
				if (extension.equals(extensionSeached))
					System.out.println(element);
			}
		...
		{
			System.out.println("Browser History Size");
			System.out.println("ArrayList: "+ url.size());	
			System.out.println("LinkedList: "+ urlLinkedList.size());
			System.out.println("HashSet: "+ urlHashSet.size());
			System.out.println("LinkedHashSet: "+ urlLinkedHashSet.size());
			System.out.println("TreeSet: "+ urlTreeSet.size());
		}
		public void urlModify(int indexModifyUrl, String modifiedUrl)
		{
			...
			urlLinkedList.set(indexModifyUrl, modifiedUrl);
			
			//modify in set not available
			urlHashSet.remove(urlToBeModified);
			urlHashSet.add(modifiedUrl);
			urlLinkedHashSet.remove(urlToBeModified);
			urlLinkedHashSet.add(modifiedUrl);
			urlTreeSet.remove(urlToBeModified);
			urlTreeSet.add(modifiedUrl);
			fetchHistory();
		}
